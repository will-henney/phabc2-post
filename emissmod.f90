...
contains
  ...
  function extinct(emtype)
    ...
    select case (emtype)
    case('neut00', 'neut01', 'FF06cm', 'PAH000')
       extinct = 0.0
  ...
  elemental function emissivity_dp(d, xi, t) result(e)
    real(DP), intent(in) :: d, xi, t
    real(DP) :: e
    real(DP) :: t4, x, lam
    real(DP), parameter :: t4floor = 3.e-4_DP
    real(DP), parameter :: xfloor = 1.0e-10_DP, xceiling = 1.0_DP - xfloor

    t4 = t*1.e-4_dp             ! routines in em2levmod want T like this

    ! set floor on the temperature
    t4 = max(t4, t4floor)
    ! and ceiling/floor on ifrac
    x = max(min(xi, xceiling), xfloor)
    ...
    select case (emtype)
       ! PAH emission now done in 
!!$    case('PAH000')
!!$       ! Generic PAH emission - a fixed fraction of the absorbed FUV radiation
    case('neut00')

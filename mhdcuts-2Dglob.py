...
if len(sys.argv) != 6:
    print "Usage: %s RUNID BSTRING TMIN TMAX TSTEP" % sys.argv[0]
    exit
...
#     imbxz = hsv_bbb_image(runid, i, jypeak, axis='y')
    ...
    im4 = Image.new(imxy.mode, (nx, 2*ny + 5*ex), color="black")
    im4.paste(imxy, (0, ny))
...
#     d.text((nx+0.5*ex, 2*ny-1.5*ex), 'x, T, rho', fill="yellow") 
    ...
    d.text((0.5*ex, 2*ny+2*ex), 'n = 1.0e4 pcc, B = (%s) mG' % bstring, 
	   fill="white") 

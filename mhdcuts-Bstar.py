...
for i in range(tmin, tmax+1, tstep):
    ...
    if isB:
        d.text((m+0.5*ex, m+0.2*ex), 'B', fill="black") 
        d.text((m+nx+m+0.5*ex, m+0.2*ex), 'B', fill="black") 
        d.text((m+2*(nx+m)+0.5*ex, m+0.2*ex), 'B', fill="black") 

    d.text((m+0.5*ex, 2*(ny+m)-1.5*ex), 'x, T, rho', fill="black") 
    d.text((m+nx+m+0.5*ex, 2*(ny+m)-1.5*ex), 'x, T, rho', fill="black") 
    d.text((m+2*(nx+m)+0.5*ex, 2*(ny+m)-1.5*ex), 'x, T, rho', fill="black") 
	...
	d.text((m, 2*(ny+m)+0.5*ex), 
	       'Run %s, t = %3.3i kyr' % (long_description[runid], i), fill="white") 
    d.text((m, 2*(ny+m)+2*ex), 'Cuts at X, Y, Z = %i' % zcut, 
	   fill="white") 
    d.text((m, 2*(ny+m)+3.5*ex), 'Arthur, Henney, de Colle, Mellema, Vazquez-Semadeni (2010)', 
	   fill="white") 

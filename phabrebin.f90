program phabrebin
  ...
  print *, 'This program rebins a sim (in current dir) to half resolution'
  ...
  forall( i = 1:nn, j = 1:nn, k = 1:nn )
     ...
     ! Do B-fields specially by averaging over area instead of volume
     newprim(5, i, j, k) = sum( prim(5, 2*i-2:2*i+1, 2*j-1:2*j, 2*k-1:2*k) ) / 8.0
     newprim(6, i, j, k) = sum( prim(6, 2*i-1:2*i, 2*j-2:2*j+1, 2*k-1:2*k) ) / 8.0
     newprim(7, i, j, k) = sum( prim(7, 2*i-1:2*i, 2*j-1:2*j, 2*k-2:2*k+1) ) / 8.0
  end forall

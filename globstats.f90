...
!!$  z = (/ (dz*(real(k) - 0.5), k = 1, nz) /)
  ...
  ! Magnetic field 
  ! CHANGE 26 Sep 2008 WJH - Take means of absolute field components
  !                          This was suggested by Fabio
  ! Total mean B
  bmeanx = sum(abs(bx))/real(nx*ny*nz)
  bmeany = sum(abs(by))/real(nx*ny*nz)
  bmeanz = sum(abs(bz))/real(nx*ny*nz)
  ! Ionized mean B
  bmeanx_i = sum(abs(bx)*xi)/sum(xi)
  bmeany_i = sum(abs(by)*xi)/sum(xi)
  bmeanz_i = sum(abs(bz)*xi)/sum(xi)
  ! Neutral mean B
  bmeanx_n = sum(abs(bx)*(1.0-xi))/sum(1.0-xi)
  bmeany_n = sum(abs(by)*(1.0-xi))/sum(1.0-xi)
  bmeanz_n = sum(abs(bz)*(1.0-xi))/sum(1.0-xi)

  ! Total RMS B
  brms = sqrt(sum(bx**2 + by**2 + bz**2)/real(nx*ny*nz))

  ! mean betas
  if (sum(bx**2 + by**2 + bz**2) /= 0.0) then

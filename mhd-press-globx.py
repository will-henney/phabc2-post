...
if varstring.startswith("pgas-pmag"):
    ...
    g = Graph(dnvar, tevar, weights=weights, gamma=0.05, statslevel=0)
...
resolutions = { '127m': 'L', '127': 'L', '255': 'H', '2d501': 'S', '2d501m': 'S', }
try: 
    first, second = runid.split('-')
    paperid = first.upper() + resolutions[second]
except ValueError:
    paperid = runid

tstring = '$t = %.0i,000$~yr' % (itime)
if itime == 0 : tstring= '$t = 0$~yr'
gtitle = r'%s, %s' % (paperid, tstring)
g.text(12*pyx.unit.x_pt, g.figheight - 9*pyx.unit.x_pt, gtitle, 
       [pyx.text.halign.left, pyx.text.valign.top])

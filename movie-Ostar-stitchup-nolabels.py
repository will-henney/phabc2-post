"""
Make 2x2 mosaic of visualization images for Ostar:
         +---------------+---------------+
         |               |               |
         |   Ostar       |  Ostar-HD     |
         |               |               |
...
phis = thetas
movienums = ["+350+350"]*ntimes + ["-0350"]*ntumbles
megayears = [float(i)/100.0 for i in itimes + [ntimes]*ntumbles]
filemetapattern = "rgb-%%s-Ostar%%s-%s%s-%4.4i.png"
# Example: rgb-CPF-Ostar-evo+350+350-0128.png
...
for filepattern, theta, phi, megayear in zip(filepatterns, thetas, phis, megayears):
    print filepattern, jcount
    savefile = "movie-Ostar-stitchup-nolabels-%3.3i.png" % (jcount)
    if os.path.isfile(savefile):
        print "%s exists - skipping!" % savefile
        jcount += 1
        continue
    rows = [[Image.open(filepattern % (chanid, runid)).resize((nx,ny)) 
              for runid in runids] for chanid in chanids]
    image = Image.new('RGB', (2*nx + 3*mx, 
                              2*ny + 3*my + runtext_height + my))
    oy = my
    ox = mx

...
g = pyxgraph.pyxgraph(xlimits=(0, worldheight), 
...
graymap = pyxgraph.ColMapper.ColorMapper("pm3d", exponent=1.0, pm3d=[3,3,3])
...
g.pyxplotarray(imagedata, colmap=mycolmap, minvalue=imagemin, maxvalue=imagemax,
	       xpos=0.0, ypos=0.0, width=worldwidth, height=worldheight, graphcoords=True)

cb = pyxgraph.pyxcolorbar(lut=mycolmap.generate_lut(), frame=g, pos=(1.1,0.0),
...
g.pyxplotcontour(vecpot, (worldwidth/nx)*(xx+1.0), (worldheight/ny)*(yy+1.0), 
		 # add 1 to positions to align with image
		 levels=50, colors='color', color=pyx.color.rgb(0.3,0.2,1.0),
		 lw=3, 
		 lineattrs=[pyx.color.transparency(0.2)]
		 )
# Plot contour at ionization front
g.pyxplotcontour(data['xn'], (worldwidth/nx)*(xx+0.5), (worldheight/ny)*(yy+0.5), 
# 		 levels=[0.1,0.5,0.9], 
		 levels=[0.5,], 
		 colors='color', 
# 		 color=pyx.color.rgb(0.2,0.5,0.0), # mid green
		 color=pyx.color.rgb(0.8,1.0,0.0), # yellow-green
		 lw=3, 
		 lineattrs=[pyx.color.transparency(0.2)]
		 )
...
y = lambda k: random.randint(int(skip)/4,int(3*skip)/4) + int(skip*(int(k)%my))
# s = lambda k: 3.e4*B[y(k),x(k)]
# s = lambda k: 0.5			# all arrows the same size 
s = lambda k: v[y(k),x(k)]# *nx/worldwidth 
